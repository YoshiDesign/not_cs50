...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for name in people:
        ...
        if people[name]['mother']:
            ...
            # find out how many expressive genes the mother has
        ...
        if people[name]['father']:
            ...
            # find out how many expressive genes the father has
...
def from_parents(mg, fg, g, t):
    ...
    from_m = 1 - PROBS['mutation'] if mg == 2 \
        else random.random() if mg == 1 \
        else PROBS['mutation']

    from_f = 1 - PROBS['mutation'] if fg == 2 \
        else random.random() if fg == 1 \
        else PROBS['mutation']
    ...
    # Formulae for dependent probabilities
    data = {
        # Two from both
        2: from_f * from_m,
        # One from either
        1: (from_f * not_from_m) + (from_m * not_from_f),
        # None from both
        0: not_from_f * not_from_m
    }
    ...
    for name in probabilities:
        for item in probabilities[name]:
            ...
            for data in probabilities[name][item]:
                probabilities[name][item][data] = probabilities[name][item][data] / t
